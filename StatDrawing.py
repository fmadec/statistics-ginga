...
class StatDrawing(GingaPlugin.LocalPlugin):

    def __init__(self, fv, fitsimage):
        # superclass defines some variables for us, like logger
        super(StatDrawing, self).__init__(fv, fitsimage)

        self.layertag = 'statdrawing-canvas'
        ...
        self.mask_obj = None
        self.maskcolor = 'yellow'
        self.maskalpha = 0.3

        # For mask creation from drawn objects
        self._drawn_tags = []
        self._mask_prefix = 'statdrawing'
    ...
    def build_gui(self, container):
        ...
        fr = Widgets.Frame("Statistics")
        vbox.add_widget(fr, stretch=0)


        captions = (('Max:', 'label', 'Max', 'llabel'),
                    ('Min:', 'label', 'Min', 'llabel'),
                    ('Mean:', 'label', 'Mean', 'llabel'),
                    ('Std:', 'label', 'Std', 'llabel'),
                    ('Sum:', 'label', 'Sum', 'llabel'),
                    ('NbPixel:', 'label', 'NbPixel', 'llabel'),
                    )
        ...
        fr = Widgets.Frame("Mask Threshold")
        vbox.add_widget(fr, stretch=0)


        captions = (('Do Threshold:', 'label', 'Do Threshold', 'checkbutton'),
                    ('Threshold:', 'label', 'Threshold', 'entry'),
                    )
        w, b = Widgets.build_info(captions, orientation=orientation)
        self.w.update(b)
        b.threshold.set_text('0')
        fr.set_widget(w)
        self.threshold = b
        b.threshold.add_callback('activated', self.do_thresold)


        vbox.add_widget(fr, stretch=0)

        fr = Widgets.Expander("Attributes")
        vbox2 = Widgets.VBox()
    ...
    def create_mask(self):
        ...
        self.statistics(old_image, mask)
    ...
    def draw_thmask_pts(self, mask):
        return

    def do_thresold(self, w):
        print(w.get_text())
        self.create_mask()
        return

    def tmp_create_mask_box(self, image, mask):
        data = image.get_data()
        shape = image.get_mddata().shape
        xx,yy = numpy.mgrid[:shape[0],:shape[1]]
        pixels = [(y,x) for x,y in zip(xx[mask],yy[mask])]
        for pixel in pixels:
            self.create_mask_box(pixel)




    def create_mask_box(self, pixel):
        x, y  = pixel

        Box = self.canvas.get_draw_class('box')
        tag = self.canvas.add(Box(x, y, 0.5, 0.5,
                                  color=self.maskcolor, alpha=0,
                                  fill=True, fillcolor=self.maskcolor,
                                  fillalpha=self.maskalpha))
 ...
 #       self.draw_cb(self.canvas, tag)
    ...
    def statistics(self,image, mask):
        threshold = float(self.threshold.threshold.get_text())
        print(threshold)
        data = image.get_data()
        doThres = False
        th_mask = mask
        if doThres:
            ...
            th_mask = numpy.logical_and(thresh, mask)

        fdata = data[th_mask]
        nbpix = numpy.count_nonzero(th_mask)
        self.wdetail.std.set_text('%.3f' % fdata.std())
        self.wdetail.mean.set_text('%.3f' % fdata.mean())
        self.wdetail.max.set_text('%.3f' % fdata.max())
        self.wdetail.min.set_text('%.3f' % fdata.min())
        self.wdetail.sum.set_text('%.3f' % fdata.sum())
        self.wdetail.nbpixel.set_text('%.3f' % nbpix)

#        self.tmp_create_mask_box(image,mask)
